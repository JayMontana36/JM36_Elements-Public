...
(
	...
	},
	{
		__call  =   function(Self)
						Self.Id             = PlayerId()
						local Ped <const>   = PlayerPedId() Self.Ped,Self.Handle=Ped,Ped
						Self.Coords         = GetEntityCoords(Ped, false)
						local IsIn <const>  = IsPedInAnyVehicle(Ped, false) Self.Vehicle.IsIn = IsIn
						if IsIn then
							local Vehicle <const> = Self.Vehicle
							...
							local VehModel <const> = GetEntityModel(Veh) Vehicle.Model = VehModel
							Vehicle.Name           = GetDisplayNameFromVehicleModel(VehModel)
							
							local Vehicle_Type <const> = Vehicle.Type
